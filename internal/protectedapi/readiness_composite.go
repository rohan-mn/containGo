...
// ReadinessDependency gives a readiness checker a readable name.
type ReadinessDependency struct {
	Name    string
	Checker ReadinessChecker
}

// CompositeReadinessChecker requires every configured dependency to be ready.
...
func (c *CompositeReadinessChecker) Check(
	...
	for _, dependency := range c.dependencies {
		if err := dependency.Checker.Check(
			ctx,
		); err != nil {
			return fmt.Errorf(
				"%s readiness check: %w",
